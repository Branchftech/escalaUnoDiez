...
class CrearObra extends ServicesComponent
{
    ...
    public function crearObra()
    {
        ...
        $this->validate([
            ...
            'estadoSeleccionado' => 'required|min:1|exists:estados,id',
            'estadoObraSeleccionado' => 'required|min:1|exists:estadoobra,id',
            'clienteSeleccionado' => 'required|min:1|exists:cliente,id',
            'proveedoresSeleccionados.*' => 'min:1|exists:proveedores,id',
            'proveedoresSeleccionados' => 'required|array',
        ]);
        try{
            $user = Auth::user();
